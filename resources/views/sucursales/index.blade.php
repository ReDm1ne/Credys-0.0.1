...
@section('header_title', 'Sucursales')
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <div class="w-full sm:w-auto order-2 sm:order-1">
                <div class="relative">
                    ...
                    <input type="search" id="search" class="block w-full p-2.5 pl-10 text-sm text-gray-900 border border-gray-300 rounded-xl bg-white focus:ring-blue-500 focus:border-blue-500 shadow-sm" placeholder="Buscar sucursales...">
                </div>
            </div>
            <a href="{{ route('sucursales.create') }}" class="w-full sm:w-auto order-1 sm:order-2 bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-5 rounded-xl transition duration-300 ease-in-out inline-flex items-center justify-center shadow-sm">
            ...
            <div class="hidden md:block overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Dirección</th>
                    ...
                    @foreach($sucursales as $sucursal)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('sucursales.show', $sucursal->id) }}" class="text-blue-600 hover:text-blue-900 mr-3 hover:underline">Ver</a>
                                <a href="{{ route('sucursales.edit', $sucursal->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3 hover:underline">Editar</a>
                                <button onclick="mostrarModalConfirmacion('{{ $sucursal->id }}')" class="text-red-600 hover:text-red-900 hover:underline">Eliminar</button>
                                <form id="form-eliminar-{{ $sucursal->id }}" action="{{ route('sucursales.destroy', $sucursal->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
            ...
            <div class="md:hidden">
                @foreach($sucursales as $sucursal)
                    <div class="border-b border-gray-200 p-4 hover:bg-gray-50">
                        <div class="mb-2">
                            <h3 class="text-lg font-semibold text-gray-900">{{ $sucursal->nombre }}</h3>
                            <p class="text-sm text-gray-600 mt-1">{{ $sucursal->direccion }}</p>
                        </div>
                        <div class="flex flex-wrap justify-end gap-2 mt-3">
                            <a href="{{ route('sucursales.show', $sucursal->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-100 text-blue-700 text-sm font-medium rounded-full hover:bg-blue-200 transition duration-200">
                            ...
                            <a href="{{ route('sucursales.edit', $sucursal->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-indigo-100 text-indigo-700 text-sm font-medium rounded-full hover:bg-indigo-200 transition duration-200">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                Editar
                            </a>
                            <button onclick="mostrarModalConfirmacion('{{ $sucursal->id }}')" class="inline-flex items-center justify-center px-4 py-2 bg-red-100 text-red-700 text-sm font-medium rounded-full hover:bg-red-200 transition duration-200">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div id="modal-confirmacion" class="fixed inset-0 bg-gray-900 bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 hidden transition-opacity duration-300 ease-in-out">
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-transform duration-300 ease-in-out scale-95 opacity-0" id="modal-content">
                <div class="p-8">
                    ...
                    <h3 class="text-xl font-bold text-center text-gray-900 mb-4">¿Eliminar sucursal?</h3>
                    <p class="text-gray-600 text-center mb-8">Esta acción no se puede deshacer. ¿Estás seguro de que deseas eliminar permanentemente esta sucursal?</p>
